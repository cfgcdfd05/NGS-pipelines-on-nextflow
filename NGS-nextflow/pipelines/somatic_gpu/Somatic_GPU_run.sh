...
echo "============================================"
echo " SOMATIC GPU VARIANT CALLING PIPELINE"
echo "============================================"
...
ls -la "$PROJECT_DIR/Somatic_GPU.nf" >&2
docker run --rm \
    "${MOUNTS[@]}" \
    -w "$PROJECT_DIR" \
    --gpus all \
    ...
    nextflow run Somatic_GPU.nf -c Somatic_GPU.config \
...
ls -lh "$RESULTS_DIR/$COHORT_NAME/bam/" 2>/dev/null || echo "  None found"
...
echo ""
echo "Variant counts:"
for vcf_file in "$RESULTS_DIR/$COHORT_NAME/vcf/"*.pass.vcf; do
    if [[ -f "$vcf_file" ]]; then
        SAMPLE=$(basename "$vcf_file" .pass.vcf)
        RAW=$(grep -vc "^#" "$RESULTS_DIR/$COHORT_NAME/vcf/${SAMPLE}.raw.vcf" 2>/dev/null || echo "0")
        FILT=$(grep -vc "^#" "$RESULTS_DIR/$COHORT_NAME/vcf/${SAMPLE}.filtered.vcf" 2>/dev/null || echo "0")
        PASS=$(grep -vc "^#" "$vcf_file" 2>/dev/null || echo "0")
        echo "  ${SAMPLE}:"
